...
function SolutionCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "card-glass p-6",
        active && "border-accent/30 shadow-card-hover"
      )}
    >
      <div className={cn("mb-4 inline-flex rounded-xl p-2.5", bg)}>
        <Icon className={cn("h-5 w-5", color)} />
      </div>
      <h3 className="type-h3 text-lg md:text-xl">{title}</h3>
    ...
    <SectionWrapper id="product" className="py-32">
      <div className="mx-auto grid max-w-7xl gap-16 px-6 lg:grid-cols-2">
        <div className="lg:sticky lg:top-32 lg:self-start">
          <p className="type-eyebrow mb-4">Problem → Solution</p>
        ...
        <div ref={containerRef} className="space-y-[40vh] py-12">
          {cards.map((card, i) => (
            <SolutionCard key={card.title} {...card} active={activeIndex === i} />
          ))}
        </div>
      </div>
    </SectionWrapper>
